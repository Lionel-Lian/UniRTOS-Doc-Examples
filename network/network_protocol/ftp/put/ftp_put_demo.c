...
#define QURL_FTP_DEMO_UPLOAD_FILENAME   "/user/ftp_test1.txt" /* Target path for upload file */
#define QURL_FTP_DEMO_UPLOAD_DATA       "welcome to test ftp" /* Test upload data */ 
...
/**
 * FTP upload data callback function
 * Read data from file and upload to FTP server
 *
 * @param buf  Data buffer pointer, used to store data read from file
 * @param size Buffer size, indicating maximum bytes that can be read
 * @param arg  User-defined parameter, pointing to FTP related information structure
 *
 * @return Actual bytes read and returned, returns 0 if read fails or no data to read
 */
unsigned long qurl_ftp_upload_data_cb(unsigned char *buf, long size, void *arg)
{
    qurl_demo_ftp_t *ftp_ptr = QOSA_NULL;
    qosa_int32_t     free_len = 0;
    qosa_int32_t     read_len = 0;
    int              ret = 0;

    ftp_ptr = (qurl_demo_ftp_t *)arg;
    /* Parameter validation, ensure FTP structure pointer is valid and buffer size is not 0 */
    if (ftp_ptr == QOSA_NULL || size == 0)
    {
        return 0;
    }

    /* Check if there is still data to upload */
    if (ftp_ptr->data.used_len < ftp_ptr->data.total_len)
    {
        /* Calculate remaining data length that can be read */
        free_len = ftp_ptr->data.total_len - ftp_ptr->data.used_len;
        /* Determine actual length to read this time, take the smaller value of buffer size and remaining data length */
        read_len = MIN(size, free_len);
        /* Read data from file to buffer */
        ret = qurl_ftp_demo_file_read(buf, ftp_ptr->data.fd, read_len);
        if (ret < 0)
        {
            return 0;
        }
        /* Update length of data already read */
        ftp_ptr->data.used_len += read_len;
        return read_len;
    }
    return 0;
}
...
/**
 * @brief Upload specified file to server via FTP protocol
 *
 * This function is used to upload local file to specified FTP server via FTP protocol.
 *
 * @param ftp_ptr Pointer to FTP control structure, containing connection information, authentication information, etc.
 * @param filename Local filename to upload (full path)
 *
 * @note Upload to current path on server
 */
int qurl_ftp_demo_put(qurl_demo_ftp_t *ftp_ptr, char *filename)
{
    ...
    data->is_file = QOSA_TRUE;  // Test uploading local file or uploading specified data

    // Open and get information of file to upload
    if (data->is_file)
    {
        data->fd = qosa_vfs_open(filename, QOSA_VFS_O_RDONLY);
        if (data->fd < 0)
        {
            QLOGE("open file err");
            return -1;
    ...
    {
        data->data_buf = QURL_FTP_DEMO_UPLOAD_DATA;
        data->total_len = qosa_strlen(QURL_FTP_DEMO_UPLOAD_DATA);
    }

    // Construct target URL: ftp://hostname/currentDir/server_filename
    qosa_free(ftp_ptr->url);
    url_len = qosa_strlen(ftp_ptr->hostname) + qosa_strlen(ftp_ptr->currentDir) + qosa_strlen(QURL_FTP_DEMO_SERVER_FILENAME) + 64;
    ...
    qosa_snprintf(ftp_ptr->url, url_len, "ftp://%s%s%s", ftp_ptr->hostname, ftp_ptr->currentDir, QURL_FTP_DEMO_SERVER_FILENAME);

    // Reset and configure qurl core parameters
    ...
    qurl_core_setopt(qurl, QURL_OPT_READ_CB, qurl_ftp_upload_data_cb);  // Set read callback function
    qurl_core_setopt(qurl, QURL_OPT_READ_CB_ARG, ftp_ptr);              // Set callback parameter
    ...
    qurl_core_setopt(qurl, QURL_OPT_NOBODY, 0L);
    qurl_core_setopt(qurl, QURL_OPT_UPLOAD_SIZE, -1L);

    // Set whether to skip PASV returned IP address based on configuration
    if (0 == ftp_ptr->skip_pasv_ip)
    {
        qurl_core_setopt(qurl, QURL_OPT_FTP_SKIP_PASV_IP, 0L);
    }

    // If active mode, set PORT option
...
/**
 * @brief FTP upload test function
 *
 * This function is used to initialize FTP test environment, and sequentially execute FTP common instruction test process,
 * including connection, directory operations, file upload, list acquisition and deletion operations.
 ...
 */
static void ftp_put_demo(void *argv)
{
    qurl_demo_ftp_t *ftp_ptr = QOSA_NULL;
    int              ret = 0;
    ...
    // Upload file test
    qurl_ftp_demo_put(ftp_ptr, QURL_FTP_DEMO_UPLOAD_FILENAME);
...
static void __unirtos_export_ftp_put_demo(void)
{
    ftp_put_demo(QOSA_NULL);
}

UNIRTOS_APP_EXPORT(200, "ftp_put_demo", __unirtos_export_ftp_put_demo);