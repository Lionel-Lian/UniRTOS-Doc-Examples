...
#define SOCKET_BLOCK_CONNECT_SERVER_ADDR  "142.250.197.132"
#define SOCKET_BLOCK_CONNECT_SERVER_PORT  80
#define SOCKET_BLOCK_CONNECT_SERVER_NAME  "www.baidu.com"
 ...
 static void qcm_socket_app_block_process(void *argv)
{
    ...
    if (qosa_dns_syn_getaddrinfo(SOCKET_BLOCK_DEMO_SIMID, SOCKET_BLOCK_DEMO_PDPID, SOCKET_BLOCK_CONNECT_SERVER_ADDR, &hints, &result) != QOSA_DNS_RESULT_OK)
    {
    ...
    while (1)
    {
        ...
        if (ret <= 0)
        {
            QLOGE("qcm_socket_read error");
            break;
        }

        QLOGD("ret = %d buff=%s", ret, buff);
    }
 ...
 */
void my_socket_block_demo_init(void)
{
    int         err = 0;
