...
/**
 * @brief SMTP TLS email sending function demonstration
 *
 * This function demonstrates how to use the qurl library to send emails through the SMTP
 * protocol, supporting SSL/TLS encrypted connections.
 * Including complete processes such as initializing the global environment, configuring
 * SMTP client parameters, setting TLS configuration, adding recipient list,
 * executing sending operations and releasing resources.
 *
 * @note SMTPS and STARTTLS can be selected for demonstration by yourself
 */
static void smtp_tls_demo(void)
{
    int              ret = 0;               /*!< Function return value */
    char            *url = QOSA_NULL;       /*!< SMTP server URL */
    qurl_core_t      core = QOSA_NULL;      /*!< qurl core handle */
    qurl_smtp_client client_ptr = {0};      /*!< SMTP client structure */
    qurl_slist_t     rcpt_list = QOSA_NULL; /*!< Recipient list */
    qurl_tls_cfg_t   tls_cfg = {0};         /*!< TLS configuration structure */

    // Set SSL type: SSL or STARTTLS (currently SSL is enabled)
    client_ptr.ssl_type = QURL_SMTP_SSL_TYPE_SSL;
    //client_ptr.ssl_type = QURL_SMTP_SSL_TYPE_STARTTLS;

    // Initialize global qurl environment
    qurl_global_init();

    // Create qurl core object
    ret = qurl_core_create(&core);

    // Configure network and authentication information (PDP ID, username, password, sender email)
    ...
    // Construct SMTP server URL (decide whether to add 's' according to SSL type)
    ...
    qosa_sprintf(url, "smtp%s://%s", QURL_SMTP_SSL_TYPE_SSL == client_ptr.ssl_type ? "s" : "", QURL_SMTP_DEMO_EMAILADDR);
    QLOGV("url: %s", url);
    ret = qurl_core_setopt(core, QURL_OPT_URL, url);
    QLOGV("ssltype: %d", client_ptr.ssl_type);

    // Initialize TLS configuration and set timeout
    qurl_tls_cfg_init(&tls_cfg);
    tls_cfg.negotiate_timeout = 30;

    // Set corresponding SMTP port number according to SSL type, different types usually have different ports
    if (client_ptr.ssl_type == QURL_SMTP_SSL_TYPE_STARTTLS)
    {
        // If STARTTLS, need to specify QURL_OPT_TLS_USETLS 1 separately
        qurl_core_setopt(core, QURL_OPT_TLS_USETLS, 1);
        ret = qurl_core_setopt(core, QURL_OPT_PORT, QURL_SMTP_DEMO_STARTTLS_PORT);
    }
    else if (client_ptr.ssl_type == QURL_SMTP_SSL_TYPE_SSL)
    {
        ret = qurl_core_setopt(core, QURL_OPT_PORT, QURL_SMTP_DEMO_TLS_PORT);
    }
    else
    {
    ...
    // Set TLS configuration to core handle
    ...
    qurl_core_setopt(core, QURL_OPT_READ_CB, qurl_smtp_upload_data_cb);
    qurl_core_setopt(core, QURL_OPT_READ_CB_ARG, (void *)&client_ptr);
    // Set timeout, if attachment is large, increase time to prevent sending failure
    qurl_core_setopt(core, QURL_OPT_TIMEOUT_MS, QURL_SMTP_DEMO_TIMEOUT * 1000);
    ...
    // Initialize SMTP client structure
...
UNIRTOS_APP_EXPORT(200, "smtp_tls_demo", smtp_tls_demo);