...
#define MQTT_DEMO_TOPIC1_ALIAS     10                            /*!< MQTT test topic 1 alias for publishing and subscribing messages */
#define MQTT_DEMO_TOPIC2_ALIAS     11                            /*!< MQTT test topic 1 alias for publishing and subscribing messages */
#define MQTT_DEMO_TOPIC3_ALIAS     12                            /*!< MQTT test topic 1 alias for publishing and subscribing messages */

#define MQTT5_RESPONSE_TOPIC1      "response_topic1"             /*!< MQTT test topic 1 for publishing and subscribing messages */
#define MQTT5_CORRELATION_DATA1    "correlation_data1"           /*!< MQTT test topic 1 for publishing and subscribing messages */
#define MQTT5_CONTENT_TYPE1        "text/plain"                  /*!< MQTT test topic 1 for publishing and subscribing messages */
#define MQTT5_CONTENT_TYPE2        "application/json"            /*!< MQTT test topic 1 for publishing and subscribing messages */

#define MQTT5_USER_PROPERTY_KEY1   "firmware"                    /*!< MQTT test topic 1 for publishing and subscribing messages */
#define MQTT5_USER_PROPERTY_VALUE1 "v1.1.0"                      /*!< MQTT test topic 1 for publishing and subscribing messages */
#define MQTT5_USER_PROPERTY_KEY2   "timestamp"                   /*!< MQTT test topic 1 for publishing and subscribing messages */
#define MQTT5_USER_PROPERTY_VALUE2 "2024-01-15T10:30:00Z"        /*!< MQTT test topic 1 for publishing and subscribing messages */
 ...
 * @brief Events reported by the MQTT client.
 */
typedef enum
{
    MQTT_DEMO_CLIENT_BEGIN_EVENT = QCM_MQTT_CLIENT_MAX_EVENT, /** Starting event on the MQTT client demo side. */
    MQTT_DEMO_CLIENT_NEW_MESSAGE_EVENT,                       /** MQTT client continues to read message events. */

} mqtt_demo_client_event_e;
...
static int unir_mqtt_demo_ctx_init(qcm_mqtt_config_t *mqtt_option)
{
    ...
    mqtt_option->version = QCM_MQTT_VERSION_V5;
    mqtt_option->conn_properties.session_expiry_interval = 60;
    mqtt_option->conn_properties.topic_alias_maximum = 60;
    mqtt_option->conn_properties.receive_maximum = 60;
    mqtt_option->conn_properties.maximum_packet_size = 1500;
    mqtt_option->conn_properties.user_property[0] = QCM_MQTT_USER_PROPERTY("key1", "value1");
    // mqtt_option->conn_properties.user_property[1] = OSA_MQTT_USER_PROPERTY("key2", "value2");
    // mqtt_option->conn_properties.user_property[2] = OSA_MQTT_USER_PROPERTY("key3", "value3");
    // mqtt_option->conn_properties.user_property[3] = OSA_MQTT_USER_PROPERTY("key4", "value4");
    // mqtt_option->conn_properties.user_property[4] = OSA_MQTT_USER_PROPERTY("key5", "value5");
    mqtt_option->conn_properties.user_property_cnt = 1;
#else
    ...
    QLOGV("mqtt version:%d", mqtt_option->version);
...
#ifdef CONFIG_QCM_MQTT5_FUNC
/**
 * @brief Get and process MQTT connection acknowledgment properties
 *
 * This function retrieves the MQTT server connection acknowledgment information
 * for the specified client ID, prints key property values, and releases
 * associated resources.
 *
 * @param client_id Client identifier used to identify the MQTT client
 *
 * @return Operation result. QCM_MQTT_RES_OK indicates success, other values indicate failure
 */
static int unir_mqtt_demo_connack_props(qosa_uint8_t client_id)
{
    int ret = 0;
    // Initialize connection acknowledgment properties structure
    qcm_mqtt_connack_properties_t connack_props = {0};

    // Get server connection acknowledgment information and check if successful
    ret = qcm_mqtt_client_get_server_info(client_id, &connack_props);
    if (ret == QCM_MQTT_RES_OK)
    {
        // Print key information from connection acknowledgment properties
        QLOGI("connack props=%d, %d, %d", connack_props.session_expiry_interval, connack_props.maximum_qos, connack_props.topic_alias_maximum);
    }

    // Release connection acknowledgment properties resources
    qcm_mqtt_release_connack_props(&connack_props);

    return ret;
}

/**
 * @brief Handle MQTT received publish message properties
 *
 * @param pub_prop Pointer to the MQTT publish properties structure
 * @return void
 */
static int unir_mqtt_demo_recvpub_props(qcm_mqtt_pub_properties_t *pub_prop)
{
    int ret = 0;
    ...
    // Print received publish properties information
    QLOGI("recvpub props=%d, %d, %d", pub_prop->message_expiry_interval, pub_prop->topic_alias, pub_prop->payload_format_indicator);

    // Release new message properties resources
    ret = qcm_mqtt_release_newmsg_props(pub_prop);

    // Free properties structure memory
    qosa_free(pub_prop);
    return ret;
}
#endif /* CONFIG_QCM_MQTT5_FUNC */
...
static void unir_mqtt_demo_publish(void)
{
    ...
    //Topic
    qcm_mqtt_malloc_data(&pub_info.topic, qosa_strlen(MQTT_DEMO_TOPIC1), MQTT_DEMO_TOPIC1);
    //payload
    qcm_mqtt_malloc_data(&pub_info.payload, qosa_strlen(MQTT_DEMO_TEST_CONTENT), MQTT_DEMO_TEST_CONTENT);

#ifdef CONFIG_QCM_MQTT5_FUNC
    pub_info.properties.message_expiry_interval = 100;  //Message Expiry Interval
    pub_info.properties.subscription_identifier = 1;    //Subscription Identifier

    if (qcm_mqtt_topic_alias_check(g_mqtt_ctx->client_id, MQTT_DEMO_TOPIC1_ALIAS) == QCM_MQTT_RES_OK)
    {
        pub_info.properties.topic_alias = MQTT_DEMO_TOPIC1_ALIAS;  //Topic Alias
    }
    pub_info.properties.payload_format_indicator = 1;              //Payload Format Indicator
    //Response Topic
    qcm_mqtt_malloc_data(&pub_info.properties.response_topic, qosa_strlen(MQTT5_RESPONSE_TOPIC1), MQTT5_RESPONSE_TOPIC1);

    //Correlation Data
    qcm_mqtt_malloc_data(&pub_info.properties.correlation_data, qosa_strlen(MQTT5_CORRELATION_DATA1), MQTT5_CORRELATION_DATA1);

    //Content Type
    qcm_mqtt_malloc_data(&pub_info.properties.content_type, qosa_strlen(MQTT5_CONTENT_TYPE1), MQTT5_CONTENT_TYPE1);

    //User Property
    qcm_mqtt_malloc_data(&pub_info.properties.user_property[0].key, qosa_strlen(MQTT5_USER_PROPERTY_KEY1), MQTT5_USER_PROPERTY_KEY1);
    qcm_mqtt_malloc_data(&pub_info.properties.user_property[0].value, qosa_strlen(MQTT5_USER_PROPERTY_VALUE1), MQTT5_USER_PROPERTY_VALUE1);
    //User Property number
    pub_info.properties.user_property_cnt = 1;
#endif /* CONFIG_QCM_MQTT5_FUNC */
    ...
    // Free properties memory
    qcm_mqtt_free_data(&pub_info.properties.response_topic);
    qcm_mqtt_free_data(&pub_info.properties.correlation_data);
    qcm_mqtt_free_data(&pub_info.properties.content_type);
    qcm_mqtt_user_property_free_data(pub_info.properties.user_property_cnt, pub_info.properties.user_property);
}
...
static void unir_mqtt_demo_subscribe(void)
{
...
#ifdef CONFIG_QCM_MQTT5_FUNC
    sub_info.properties.subscription_identifier = 1;  //Subscription Identifier

    //User Property
    qcm_mqtt_malloc_data(&sub_info.properties.user_property[0].key, qosa_strlen(MQTT5_USER_PROPERTY_KEY1), MQTT5_USER_PROPERTY_KEY1);
    qcm_mqtt_malloc_data(&sub_info.properties.user_property[0].value, qosa_strlen(MQTT5_USER_PROPERTY_VALUE1), MQTT5_USER_PROPERTY_VALUE1);

    //User Property number
    sub_info.properties.user_property_cnt = 1;
#endif /* CONFIG_QCM_MQTT5_FUNC */
    ...
    // Free User Property memory
    qcm_mqtt_user_property_free_data(sub_info.properties.user_property_cnt, sub_info.properties.user_property);
...
static void unir_mqtt_demo_unsubscribe(void)
{
...
#ifdef CONFIG_QCM_MQTT5_FUNC
    //User Property
    qcm_mqtt_malloc_data(&un_sub_info.properties.user_property[0].key, qosa_strlen(MQTT5_USER_PROPERTY_KEY1), MQTT5_USER_PROPERTY_KEY1);
    qcm_mqtt_malloc_data(&un_sub_info.properties.user_property[0].value, qosa_strlen(MQTT5_USER_PROPERTY_VALUE1), MQTT5_USER_PROPERTY_VALUE1);

    //User Property number
    un_sub_info.properties.user_property_cnt = 1;
#endif /* CONFIG_QCM_MQTT5_FUNC */
    ...
    // Free User Property memory
    qcm_mqtt_user_property_free_data(un_sub_info.properties.user_property_cnt, un_sub_info.properties.user_property);
...
{
    qcm_mqtt_eercode_e     ret = 0;
    qcm_mqtt_disc_config_t disconn_info = {0};

#ifdef CONFIG_QCM_MQTT5_FUNC
    //User Property
    qcm_mqtt_malloc_data(&disconn_info.properties.user_property[0].key, qosa_strlen(MQTT5_USER_PROPERTY_KEY1), MQTT5_USER_PROPERTY_KEY1);
    qcm_mqtt_malloc_data(&disconn_info.properties.user_property[0].value, qosa_strlen(MQTT5_USER_PROPERTY_VALUE1), MQTT5_USER_PROPERTY_VALUE1);

    //User Property number
    disconn_info.properties.user_property_cnt = 1;
#endif /* CONFIG_QCM_MQTT5_FUNC */
    /* Call MQTT client disconnect interface */
    ret = qcm_mqtt_client_disconnect(g_mqtt_ctx->client_id, &disconn_info);
    if (ret != QCM_MQTT_RES_OK)
    {
        QLOGE("ret=%x", ret);
    }
    ...
    // Free User Property memory
    qcm_mqtt_user_property_free_data(disconn_info.properties.user_property_cnt, disconn_info.properties.user_property);
}
...
static int unir_mqtt_demo_event_handle(mqtt_demo_msg_t *msg)
{
    ...
    else if (msg->msg_id == QCM_MQTT_CLIENT_CONNECT_EVENT)
    {
        ...
        {
#ifdef CONFIG_QCM_MQTT5_FUNC
            unir_mqtt_demo_connack_props(client_id);
#endif /* CONFIG_QCM_MQTT5_FUNC */
...
/**
 * @brief Check the number of cached messages in the MQTT client, and notify the upper layer application if there are cached messages
 *
 * @param client_id MQTT client identifier
 *
 * @return No return value
 */
static void unir_mqtt_demo_check_cache_cnt(qosa_uint8_t client_id)
{
    qosa_uint8_t cache_cnt = 0;

    // Get the number of messages waiting to be read for the specified client
    cache_cnt = qcm_mqtt_client_wait_read_cnt(client_id);
    QLOGV("cache_cnt=%d", cache_cnt);
    if (cache_cnt > 0)
    {
        qcm_mqtt_common_resp_t *msg = QOSA_NULL;
        // Allocate memory for the message notification structure
        msg = qosa_malloc(sizeof(qcm_mqtt_common_resp_t));
        if (msg == QOSA_NULL)
        {
            QLOGE("malloc err");
            return;
        }
        // Fill in the client ID and trigger the new message event callback
        msg->client_id = client_id;
        unir_mqtt_demo_event_cb(MQTT_DEMO_CLIENT_NEW_MESSAGE_EVENT, msg, QOSA_NULL);
    }
}
...
static void unir_mqtt_demo_recv_new_message(mqtt_demo_msg_t *msg)
{
...
#ifdef CONFIG_QCM_MQTT5_FUNC
        //print MQTT publish properties
        unir_mqtt_demo_recvpub_props(recv_pub.properties);
#endif /* CONFIG_QCM_MQTT5_FUNC */
    ...
    unir_mqtt_demo_check_cache_cnt(client_id);
}

/**
 * @brief Read MQTT received messages
 */
static void unir_mqtt_demo_recv_cache_message(mqtt_demo_msg_t *msg)
{
    qosa_uint8_t        client_id = 0;       // Client ID
    qosa_uint32_t       payload_len = 0;     // Message length
    qcm_mqtt_recv_pub_t recv_pub = {0};      // Topic
    ...
    QLOGD("new_message=%d", client_id);
    // Read subscribed message content from MQTT client
    if (qcm_mqtt_client_read_cacha_data(client_id, &recv_pub) == QCM_MQTT_RES_OK)
    {
        // If URC output is enabled, send message related information via URC
        if (MQTT_DEMO_TEST_URC_ENABLE)
        {
    ...
    while (1)
    {
        ...
        switch (msg.msg_id)
        {
            ...
            case QCM_MQTT_CLIENT_CLOSE_EVENT:
                ret = unir_mqtt_demo_event_handle(&msg);
                break;
            case MQTT_DEMO_CLIENT_NEW_MESSAGE_EVENT:
                unir_mqtt_demo_recv_cache_message(&msg);
                break;
            default:
                break;
        }
 ...
 */
void unir_mqtt5_demo_init(void)
{
    int         err = 0;
