...
//qosa_sms_demo_send_ascii_characters_sms("10086", "sms test")
static int qosa_sms_demo_send_ascii_characters_sms(const char *phone_number, const char *message_txt)
{
    ...
    // DATA support QOSA_CS_GSM charset
    message.text.send.data_chset = QOSA_CS_GSM;
    ...
    // If the messages is GSM encoding. the PDU is configured as 7bit, the dcs set 0x00
    message.text.send.dcs = 0x00;
